...
try:
    ...
    if sys.platform == 'win32':
        if platform.architecture()[0] == "32bit":
            libsvm = CDLL(path.join(dirname, r'.\libsvm_x86.dll'))
        elif platform.architecture()[0] == "64bit":
            libsvm = CDLL(path.join(dirname, r'.\libsvm_x64.dll'))
    else:
        if platform.architecture()[0] == "linux":
            libsvm = CDLL(path.join(dirname, './libsvm_linux.so.2'))
        else:
            libsvm = CDLL(path.join(dirname, './libsvm_macos.so.2'))
except:
